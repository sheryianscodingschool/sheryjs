...
export const init = (
  ...
  Object.assign(uniforms, {
    ...
    shapeEdgeSoftness: { value: 0.0, range: [0, 1] },
  ...
  const shapeModifier = `uniform float shapeRadius,shapeEdgeSoftness;uniform vec2  shapePosition,shapeScale;  void roundedBoxSDF() {    float distance = length(max(abs(vuv-shapePosition*2.0-vec2(.5,.5)) - shapeScale + shapeRadius, 0.0)) - shapeRadius;    float smoothedAlpha = 1.0-smoothstep(0.0, shapeEdgeSoftness * 2.0, distance);    gl_FragColor = vec4(gl_FragColor.xyz, smoothedAlpha);}`
  ...
  if ((opts.debug && !isdebug[effect]) || false) {
    ...
    panel = controlKit
      ...
      .addPad(debugObj, 'shapePosition', {
        label: "Shape Position",
        onChange: () => uniforms.shapePosition.value.set(...debugObj.shapePosition)
      })
