function Shery() {
    ...
    return {
        ...
        popImagesOnMouseMove: function (element = null, opts = {}) {
            var elem;
            if (!element) elem = window;
            else elem = document.querySelector(element);
            ...
            elem.addEventListener("mousemove", throttleFunction(function(dets){
                var div = document.createElement("div");
                div.classList.add("opener");
                div.style.top = `${dets.clientY}px`;
                div.style.left = `${dets.clientX}px`;
                var rotateval;
                var elemhovering = elem.getBoundingClientRect();
                if(dets.clientX>elemhovering.left + elemhovering.width/2){
                    rotateval = gsap.utils.clamp(10, 20, Math.floor(Math.random() * (20 - (-10) + 1) + -10));
                }
                else{
                    rotateval = gsap.utils.clamp(10, 20, Math.floor(Math.random() * (20 - (-10) + 1) + -10));
                    rotateval = rotateval*-1;
                }
                div.style.transform = `translate(-50%,-50%) rotate(${rotateval}deg)`;
                var img = document.createElement("img");
                img.setAttribute("src", opts.images[Math.floor(Math.random()*opts.images.length)]);
                img.style.transform = `translateY(100%)`;
                img.style.width = "150px";
                div.appendChild(img)
                document.body.appendChild(div);
                gsap.to(div, {
                    x: gsap.utils.clamp(-60,60, Math.floor(Math.random() * (60 - (-60) + 1) + -60))+'%',
                    x: gsap.utils.clamp(-60,60, Math.floor(Math.random() * (60 - (-60) + 1) + -60))+'%',
                    ease: Expo.easeOut,
                    duration: 2.5
                })
        ...
        makeParallax: function(element, opts={}){
